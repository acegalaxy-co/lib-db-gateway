...
function isMutation(method: string, url: string): boolean {
  ...
  if (m === "DELETE" || m === "PATCH") return true;
  if (m !== "POST") return false;
  if (url.endsWith("/query")) return false;
  if (url.endsWith("/search")) return false;
  return true;
}

function resourceFromUrl(url: string): any {
  const m = url.match(/\/v1\/(pages|databases|blocks|users)\/?([^/?]+)?/);
  if (!m) return { type: "unknown" };
  return { type: m[1], id: m[2] || null };
}

function extractSummary(method: string, body: any): any {
  if (!body || typeof body !== "string") return null;
  try {
    const obj = JSON.parse(body);
    if (method === "PATCH") {
      if (obj.properties) return { kind: "props-update", fields: Object.keys(obj.properties) };
      if (obj.archived !== undefined) return { kind: "archive", archived: obj.archived };
      if (obj.children) return { kind: "blocks-append", count: obj.children.length };
    }
...
function _lineSkipped(line: string, callerSkip: (string | RegExp)[]): boolean {
...
function callerInfo(callerSkip?: (string | RegExp)[]): any {
  const skip = callerSkip || [];
  const stack = (new Error()).stack || "";
  for (const line of stack.split("\n").slice(2)) {
    if (
      line.includes("/adapters/notion/")
...
function auditLog(auditDir: string | undefined, record: any): void {
  if (!auditDir) return;
  try {
...
function createNotionClient(partialCfg?: any): any {
  ...
  function _resolveUrl(urlOrPath: string): string {
  ...
  async function request(urlOrPath: string, options: any = {}): Promise<any> {
    ...
    for (let attempt = 0; attempt <= cfg.maxRetries; attempt++) {
      try {
        ...
        if (resp.ok) {
          if (isMut) auditLog(cfg.auditDir, { ...auditBase, ok: true, status: resp.status, durationMs: Date.now() - startTime });
          finalResp = resp;
          break;
        }
        if (resp.status === 429) {
    ...
    if (finalResp) {
      if (cfg.hooks && cfg.hooks.afterResponse) {
        try { cfg.hooks.afterResponse(hookCtx, url, options, finalResp); } catch (_) { /* never throw */ }
      }
      return finalResp;
    }

    if (isMut) auditLog(cfg.auditDir, { ...auditBase, ok: false, error: lastErr, durationMs: Date.now() - startTime });
    ...
    if (cfg.hooks && cfg.hooks.afterResponse) {
      try { cfg.hooks.afterResponse(hookCtx, url, options, failResp); } catch (_) { /* never throw */ }
    }
    return failResp;
  }

  async function queryAll(dbId: string, filter?: any, propertyIds?: string[]): Promise<any[]> {
    const pages: any[] = [];
    ...
    while (hasMore) {
      ...
      if (hasMore) await sleep(cfg.batchDelayMs);
    }
    return pages;
  }
  ...
  async function updatePage(pageId: string, properties: any): Promise<boolean> {
    const resp = await request(`${cfg.baseUrl}/v1/pages/${pageId}`, {
      method: "PATCH", body: JSON.stringify({ properties }),
    });
    if (!resp.ok) {
      let detail = "";
      try { const j = await resp.json(); detail = j.message || j.code || ""; } catch {}
