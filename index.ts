...
// types.ts exports interfaces only (compile-time); runtime require not needed.
...
type PolicyEntry = import("./types").PolicyEntry;
...
function _resolveAdapter(store: "postgres" | "sqlite" | "notion", val: unknown): any {
  // An object already implementing execute() is used as-is (test injection / pre-built adapter).
  if (val && typeof (val as any).execute === "function") return val;
  const opts: Record<string, unknown> = val && typeof val === "object" ? (val as Record<string, unknown>) : {};
  switch (store) {
    case "notion":
 ...
 */
function createDbGateway(opts: DbGatewayOptions = {}): DbGateway {
  const dryRun = !!opts.dryRun;
  const policies: Record<string, PolicyEntry> | undefined = opts.policies;
  ...
  const adapters: Map<string, any> = new Map();
  if (opts.adapters) {
    for (const store of Object.keys(opts.adapters) as Array<"postgres" | "sqlite" | "notion">) {
      const inst = _resolveAdapter(store, (opts.adapters as Record<string, unknown>)[store]);
      if (inst) adapters.set(store, inst);
    }
  }

  async function _finalize(
    outcome: import("./types").OutcomeRecord,
    started: number
  ): Promise<{ outcome: "allow" | "deny"; denyReason: string | null; latencyMs: number }> {
   ...
   */
  async function query(request: QueryRequest, caller: Caller): Promise<QueryResult> {
    const started: number = Date.now();

    const outcome: import("./types").OutcomeRecord = {
      ts: _nowIso(),
...
let _defaultGateway: DbGateway | null = null;
function _getDefaultGateway(): DbGateway {
  if (!_defaultGateway) {
...
export = { createDbGateway, query, IDBAdapter };
